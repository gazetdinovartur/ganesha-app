...
3. Подтверждение имени (или ввод нового)
...
1. «начать» → приветствие  
2. «меню» → список дат → отправить дату `YYYY-MM-DD`  
3. Отправить номер блюда (`1`, `2`, …) — добавить в корзину  
4. «корзина» → «оформить»  
5. Отправить телефон `+79123456789` (если ещё не сохранён)  
6. Получить номер заказа и реквизиты оплаты  

Команды: «меню», «корзина», «оформить», «повтор {token}».
